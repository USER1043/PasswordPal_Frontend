...
pub fn encrypt_entry_logic(st: &VaultState, entry: &VaultEntry) -> Result<String, String> {
    ...
    let cipher =
        Aes256Gcm::new_from_slice(key_bytes).map_err(|_| "Failed to initialize the cipher")?;
...
pub fn decrypt_entry_logic(st: &VaultState, blob_b64: &str) -> Result<VaultEntry, String> {
    ...
    let payload = general_purpose::STANDARD
        .decode(blob_b64)
        .map_err(|_| "Invalid base64 encrypted blob")?;
    ...
    let plaintext_bytes = cipher
        ...
        .map_err(|_| "Failed to decrypt data (wrong key or corrupted data)")?;

    let entry: VaultEntry =
        serde_json::from_slice(&plaintext_bytes).map_err(|_| "Decrypted JSON is invalid")?;
